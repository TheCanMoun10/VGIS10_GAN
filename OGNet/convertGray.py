...
for filename in os.listdir(origin_path):
    ...
    final_path = destin_path + f"{new_name}.png"
    status = cv2.imwrite(final_path, img)
...
for filename in os.listdir(test_origin_path):
    ...
    _, tail = os.path.split(file_path) 
    new_name = os.path.splitext(tail)[0]
    final_path = test_destin_path + f"{new_name}.png"
    status = cv2.imwrite(final_path, img)
...
for filename in os.listdir(test_origin_path2):
    ...
    _, tail = os.path.split(file_path) 
    new_name = os.path.splitext(tail)[0]
    final_path = test_destin_path2 + f"{new_name}.png"
    status = cv2.imwrite(final_path, img)
