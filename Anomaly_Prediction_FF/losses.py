...
class Wasserstein_Loss(nn.Module):
    ...
    def forward(self, real_outputs, fake_outputs):
        return torch.mean(real_outputs * fake_outputs)
...
class Perceptual_Loss(nn.Module):
    def __init__(self, layer_ids):
        ...
        self.vgg19 = models.vgg19(pretrained=True).features[:max(layer_ids)+1]
        for param in self.vgg19.parameters():
            param.requires_grad = False
        self.criteria = nn.L1Loss()
        
    def forward(self, gen_frames, gt_frames):
        x_vgg, y_vgg = self.vgg19(gen_frames), self.vgg19(gt_frames)
        loss = 0
        for i, layer_id in enumerate(self.layer_ids):
            loss += self.criteria(x_vgg[i], y_vgg[i])
        return loss
