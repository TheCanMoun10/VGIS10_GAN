...
class Label_loader:
    def __init__(self, cfg, video_folders):
        ...
        self.mat_path = f'{cfg.data_root}/{self.name}.mat'
        self.video_folders = video_folders

    def __call__(self):
        if self.name == 'shanghaitech':
            gt = self.load_shanghaitech()
        elif self.name == 'IITB_corridor':
            gt = self.load_IITBCorridor()
        else:
    ...
    def load_ucsd_avenue(self):
        ...
        for i in range(abnormal_events.shape[0]):
            length = len(os.listdir(self.video_folders[i])) - 1
    ...
    def load_IITBCorridor(self):
        np_list = glob.glob(f'{self.cfg.data_root}frame_mask/*.npy')
        np_list.sort()
