...
from .memory_final_spatial_sumonly_weight_ranking_top1_mem_dist import *
...
class convAE(torch.nn.Module):
    ...
    def forward(self, x, keys,train=True):
        ...
        if train:
            updated_fea, keys, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, mem_dist_loss = self.memory(fea, keys, train)
            output = self.decoder(updated_fea, skip1, skip2, skip3)
            
            return output, fea, updated_fea, keys, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, mem_dist_loss
