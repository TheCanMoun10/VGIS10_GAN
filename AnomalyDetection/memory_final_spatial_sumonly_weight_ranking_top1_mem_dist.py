...
class Memory(nn.Module):
    def __init__(self, memory_size, feature_dim, key_dim,  temp_update, temp_gather):
        ...
        self.ideal = torch.zeros((1024,10))
        for i in range(1024):
          self.ideal[i,i%10] = 1
        self.ideal = self.ideal.cuda()
    ...
    def forward(self, query, keys, train=True):
        ...
        if train:
            #losses
            separateness_loss, compactness_loss, mem_dist_loss = self.gather_loss(query,keys, train)
            # read
            updated_query, softmax_score_query,softmax_score_memory = self.read(query, keys)
            #update
            updated_memory = self.update(query, keys, train)
            
            return updated_query, updated_memory, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, mem_dist_loss
    ...
    def gather_loss(self,query, keys, train):
        ...
        if train:
            ...
            return gathering_loss, top1_loss, mem_dist_loss
