...
f = open(os.path.join(log_dir, f'loss{args.neg_loss}_log.txt'),'w')
sys.stdout= f
...
m_items = F.normalize(torch.rand((args.msize, args.mdim), dtype=torch.float), dim=1).to(device) # Initialize the memory items
...
torch.save(model, os.path.join(log_dir, f'loss{args.neg_loss}_model.pth'))
torch.save(m_items, os.path.join(log_dir, f'loss{args.neg_loss}_keys.pt'))
